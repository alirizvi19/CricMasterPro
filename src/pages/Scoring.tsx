...
export default function Scoring() {
  ...
  const [pendingBall, setPendingBall] = useState<{ runs: number, extraType: string | null, isWicket: boolean } | null>(null);
  ...
  const completeMatch = async () => {
    if (!match || !id) return;
    if (!window.confirm("Complete match and update career stats? This cannot be undone.")) return;
    setLoading(true);
    try {
      ...
      const scoreA = match.score.teamA.runs;
      const scoreB = match.score.teamB.runs;
      await updateDoc(doc(db, 'matches', id), {
        status: 'completed',
        winnerId: scoreA > scoreB ? match.teamAId : (scoreB > scoreA ? match.teamBId : 'tie')
      });
  ...
  const recordBall = async (runs: number, extraType: string | null = null, isWicket: boolean = false) => {
    if (!match) return;
  ...
  const handleSelectNewBatsman = async (player: any) => {
    ...
    try {
      const updateData: any = {
        ...
        strikerRole: player.role || 'batsman',
        strikerPhotoUrl: player.photoUrl || null,
        strikerRuns: 0,
        strikerBalls: 0
      };
      ...
      // If the over also ended, show bowler selection now
      const isOverEnded = Math.round((match.score[battingTeamKey].overs % 1) * 10) === 6;
      if (isOverEnded) {
        setShowBowlerSelection(true);
      }
    } catch (err) {
  ...
  if (!isAuthorized) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-6 bg-bg-primary p-6">
        ...
        <div className="text-center space-y-2">
          <h2 className="text-3xl font-black uppercase text-white italic tracking-tighter">Access Denied</h2>
          <p className="text-text-dim text-[10px] font-bold uppercase tracking-[0.2em] max-w-[280px]">
            Only the tournament organizer or assigned scorers can update this match.
          </p>
        </div>
        <button 
          onClick={() => navigate(-1)}
          className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl text-white text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 transition-all active:scale-95"
        >
  ...
  return (
    <div className="max-w-md mx-auto h-[100dvh] flex flex-col bg-[#1A1A1A] text-white">
      {/* Header */}
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <section className="relative px-8 py-10 flex flex-col items-center justify-center bg-[url('https://images.unsplash.com/photo-1540747913346-19e32dc3e97e?q=80&w=2000&auto=format&fit=crop')] bg-cover bg-center">
          ...
          <div className="relative z-10 text-center space-y-2">
            <div className="flex items-center justify-center gap-1">
              ...
              <span className="text-6xl font-black italic tracking-tighter leading-none">/</span>
              <motion.span 
              ...
              <motion.span 
                ...
                initial={{ opacity: 0.5 }}
                animate={{ opacity: 1 }}
                className="text-base font-bold text-white/50 mb-4 tracking-tighter"
              >
          ...
          {match.isFreeHit && (
            <motion.div 
              ...
              className="absolute top-4 right-4 bg-brand text-black px-2 py-0.5 rounded-full text-[8px] font-black uppercase italic shadow-lg"
            >
              Free Hit
            </motion.div>
          )}
        ...
        <div className="grid grid-cols-2 divide-x divide-white/5 bg-[#242424] border-y border-white/5">
          <div className="p-3 pl-4 flex flex-col gap-0.5 bg-white/5">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-black uppercase italic text-brand leading-none">{match.strikerName || 'Striker'}</span>
                {match.strikerRole && (
          ...
          <div className="p-3 pl-4 flex flex-col gap-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-black uppercase italic text-white/50 leading-none">{match.nonStrikerName || 'Non-Striker'}</span>
        ...
        <div className="bg-[#1A1A1A] p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col">
                <span className="text-[11px] font-black uppercase italic leading-none">{match.bowlerName || 'Bowler'}</span>
                {match.bowlerRole && (
      ...
      {(isInningsCompleted || isMatchCompleted) ? (
        ...
        <div className="grid grid-cols-4 bg-white border-t border-black/5">
          ...
          <SideBtn 
            label="UNDO" 
            color={balls.length > 0 ? "text-red-500" : "text-text-dim/20"} 
            onClick={handleUndo} 
          />
          ...
          <SideBtn label="5, 7" onClick={() => setShowSpecialRuns(true)} />

          <GridBtn value="WD" onClick={() => handleRecordBall(0, 'wide')} small />
          <GridBtn value="NB" onClick={() => handleRecordBall(0, 'nb')} small />
          <GridBtn value="BYE" onClick={() => handleRecordBall(0, 'bye')} small />
          <SideBtn label="OUT" color="text-[#D0021B]" onClick={() => handleRecordBall(0, null, true)} />

          <div className="col-span-3 bg-[#8E9AAF]/10 h-12 flex items-center justify-center gap-2 border-r border-black/5">
            <span className="text-[10px] font-bold uppercase tracking-widest text-[#4A4E69]">Scoring Shortcuts</span>
            <div className="w-4 h-4 border-2 border-[#4A4E69] border-t-0 border-l-0 rotate-[-135deg] mb-1" />
          </div>
          <GridBtn value="LB" onClick={() => handleRecordBall(0, 'lb')} small />
        </div>
      )}
        ...
        {pendingBall && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="text-center space-y-2">
                <h3 className="text-xl font-black uppercase italic italic text-white tracking-tight">Confirm Event?</h3>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  onClick={() => {
                    recordBall(pendingBall.runs, pendingBall.extraType, pendingBall.isWicket);
                    setPendingBall(null);
                  }}
...
function GridBtn({ value, label, onClick, small }: { value: string, label?: string, onClick: () => void, small?: boolean }) {
  return (
    <motion.button
      whileTap={{ scale: 0.98, backgroundColor: '#F8F9FA' }}
      onClick={onClick}
      className="h-[76px] flex flex-col items-center justify-center border-r border-b border-black/5 bg-white"
    >
      <span className={`${small ? 'text-sm' : 'text-xl'} font-black italic text-[#343A40]`}>{value}</span>
      {label && <span className="text-[7px] font-black uppercase text-[#ADB5BD] -mt-0.5 tracking-tighter">{label}</span>}
    </motion.button>
  );
}

function SideBtn({ label, color = "text-[#343A40]", onClick }: { label: string, color?: string, onClick: () => void }) {
  return (
    <motion.button
      whileTap={{ scale: 0.98, backgroundColor: '#F8F9FA' }}
      onClick={onClick}
      className={`h-[76px] flex items-center justify-center border-b border-black/5 bg-[#F8F9FA] ${color} font-black uppercase italic text-[11px] tracking-tight`}
    >
      {label}
    </motion.button>
  );
