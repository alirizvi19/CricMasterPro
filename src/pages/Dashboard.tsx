    ...
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tighter text-white italic">My Dashboard</h1>
          <p className="text-text-dim font-medium">Manage your tournaments, teams and player performance</p>
        </div>
        <button
          onClick={() => setShowCreateModal(true)}
          className="bg-brand text-black px-6 py-4 rounded-xl font-bold uppercase tracking-widest text-xs flex items-center gap-2 hover:scale-105 transition-transform shadow-lg shadow-brand/20"
        >
      ...
      <section className="space-y-6">
        ...
        {tournaments.length === 0 ? (
          <div className="py-20 text-center bg-white/5 border-2 border-dashed border-white/5 rounded-3xl">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <motion.div
            ...
            className="bg-bg-secondary rounded-3xl p-8 w-full max-w-md border border-white/10"
          >
            ...
            <form onSubmit={handleCreateTournament} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                <button
                  ...
                  className="flex-1 py-3 border border-white/10 rounded-xl font-bold uppercase text-[10px] tracking-widest text-white hover:bg-white/5 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 bg-brand text-black rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-white transition-colors"
                >
                  Create
                </button>
              </div>
  ...
  return (
    <div className="relative group/card h-full">
      <Link to={`/tournament/${tournament.id}`}>
        <motion.div
          whileHover={{ y: -8 }}
          className="bg-bg-secondary border border-white/5 rounded-[2.5rem] p-8 hover:border-brand/50 transition-all group h-full flex flex-col justify-between shadow-2xl relative overflow-hidden"
        >
